...
function isUndoable(m){return !m.undone && !Array.isArray(m.items) && /^\d{3}\.\d{3}$/.test(String(m.ref||'')) && Number(m.qty)>0 && (m.type==='entrada'||m.type==='saida')}

function render(){renderStatus();renderComponents();renderSelect();renderMovementItems();renderHistory();$('#scriptUrl').value=state.config.scriptUrl||'';$('#alertEmail').value=state.config.alertEmail||'gerencia@gosmile.pt'}
...
function componentLabel(ref){const c=state.components.find(x=>x.ref===ref);return c?`${c.ref} · ${c.measure}`:ref}
function renderMovementItems(){const box=$('#movementItems');if(!box)return;box.innerHTML=movementItems.length?movementItems.map((it,i)=>`<div class="movement-item"><div><strong>${esc(componentLabel(it.ref))}</strong><small>${Number(it.qty)} un.</small></div><button type="button" class="remove-line" data-line="${i}">×</button></div>`).join(''):'<p class="hint compact">Adiciona um ou vários componentes antes de registar.</p>'}
function addMovementItem(){const ref=$('#componentSelect').value;const qty=Number($('#quantity').value);if(!ref||!qty||qty<1){toast('Indica componente e quantidade.');return}const existing=movementItems.find(x=>x.ref===ref);if(existing)existing.qty+=qty;else movementItems.push({ref,qty});$('#quantity').value=1;renderMovementItems()}
function movementSummary(m){const total=Number(m.qty||0);if(Array.isArray(m.items)&&m.items.length){return `${m.type==='entrada'?'Entrada':'Saída'} · ${m.items.length} refs · ${total} un.${m.undone?' · ANULADO':''}`}return `${m.type==='entrada'?'Entrada':'Saída'} · ${esc(m.ref)} · ${total} un.${m.undone?' · ANULADO':''}`}
function movementLines(m){return Array.isArray(m.items)&&m.items.length?`<div class="movement-lines">${m.items.map(it=>`<span>${esc(it.ref)} · ${Number(it.qty)} un.</span>`).join('')}</div>`:''}
function renderHistory(){
  const items=state.history.map((m,index)=>({...m,index})).filter(m=>historyFilter==='todos'||m.type===historyFilter).reverse();
  $('#historyList').innerHTML=items.map(m=>`<div class="movement ${m.type==='entrada'?'movement-in':'movement-out'} ${m.undone?'undone':''}"><div class="movement-main"><strong>${movementSummary(m)}</strong>${movementLines(m)}<small>${new Date(m.ts).toLocaleString('pt-PT')} · ${esc(m.location||'')} ${m.patient?'· Paciente: '+esc(m.patient):''}</small>${m.notes?`<p>${esc(m.notes)}</p>`:''}${m.photo?'<small>📷 foto registada</small>':''}</div>${isUndoable(m)?`<button class="undo-btn" data-index="${m.index}" type="button">↶ Undo</button>`:''}</div>`).join('')||`<p class="hint">Sem ${historyFilter==='entrada'?'entradas':historyFilter==='saida'?'saídas':'movimentos'}.</p>`;
}
...
function applyLocal(m){const items=Array.isArray(m.items)&&m.items.length?m.items:[{ref:m.ref,qty:m.qty}];for(const it of items){const comp=state.components.find(c=>c.ref===it.ref);if(!comp)throw new Error('Componente não encontrado: '+it.ref);const q=Number(it.qty);if(m.type==='saida'&&q>Number(comp.current))throw new Error('Stock insuficiente: '+it.ref)}for(const it of items){const comp=state.components.find(c=>c.ref===it.ref);const q=Number(it.qty);if(m.type==='saida'){comp.current=Number(comp.current)-q}else{comp.current=Number(comp.current)+q;comp.initial=Number(comp.initial||0)+q}it.stockAfter=comp.current}m.ref=items.map(it=>it.ref).join(' + ');m.qty=items.reduce((sum,it)=>sum+Number(it.qty||0),0);m.stockAfter=items.map(it=>`${it.ref}: ${it.stockAfter}`).join('; ');state.history.push(m);save()}

$('#movementForm').addEventListener('submit',async e=>{e.preventDefault();const type=$('#movementType').value;const items=movementItems.length?movementItems.map(x=>({...x})):[{ref:$('#componentSelect').value,qty:Number($('#quantity').value)}];const totalQty=items.reduce((sum,it)=>sum+Number(it.qty||0),0);const m={id:crypto.randomUUID?.()||String(Date.now()),ts:new Date().toISOString(),type,ref:items.map(it=>it.ref).join(' + '),qty:totalQty,items,patient:$('#patient').value.trim(),location:$('#location').value,notes:$('#notes').value.trim(),photo:photoData};if(type==='saida'&&!m.patient){toast('Indica o nome do paciente.');return}if(!items.length||items.some(it=>!it.ref||Number(it.qty)<1)){toast('Adiciona pelo menos um componente válido.');return}try{if(state.config.scriptUrl){await sendMovement(m);state.history.push(m);save()}else{applyLocal(m);if(items.some(it=>{const comp=state.components.find(c=>c.ref===it.ref);return comp&&Number(comp.current)<2}))toast('Stock abaixo de 2 — email activo só com backend.')}movementItems=[];photoData='';$('#photoPreview').hidden=true;e.target.reset();$('#quantity').value=1;render();toast('Movimento registado')}catch(err){toast(err.message)}});
...
$('#addComponent').addEventListener('click',addMovementItem);
$('#movementItems').addEventListener('click',e=>{const btn=e.target.closest('.remove-line');if(!btn)return;movementItems.splice(Number(btn.dataset.line),1);renderMovementItems()});
...
$('#exportBtn').addEventListener('click',()=>{const header=['data','tipo','referencia','quantidade','paciente','local','notas','stock_apos','componentes','anulado'];const rows=state.history.map(m=>[m.ts,m.type,m.ref,m.qty,m.patient||'',m.location||'',m.notes||'',m.stockAfter||'',Array.isArray(m.items)?m.items.map(it=>`${it.ref} x${it.qty}`).join('; '):'',m.undone?'sim':'']);const csv=[header,...rows].map(r=>r.map(x=>'"'+String(x).replaceAll('"','""')+'"').join(',')).join('\n');const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));a.download='gosmile-stock-movimentos.csv';a.click()});
let lastAutoSync=0;
