...
async function syncFromBackend(){state.config.scriptUrl=DEFAULT_BACKEND_URL;try{const res=await fetch(state.config.scriptUrl+'?action=list&ts='+Date.now(),{cache:'no-store'});const data=await res.json();if(data.components){state.components=data.components;save();render();toast('Stock sincronizado com a Google Sheet')}}catch(e){$('#backendStatus').textContent='Erro ligação';toast('Falha ao sincronizar: '+e.message)}}
...
if('serviceWorker'in navigator){navigator.serviceWorker.register('./sw.js').catch(()=>{})}render();syncFromBackend();
